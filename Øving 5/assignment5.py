...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        if all([len(x) == 1 for x in assignment.values()]): return assignment
    ...
    def inference(self, assignment, queue):
        ...
        while len(queue):
            ...
            if self.revise(assignment, i, j):
                if not len(assignment[i]): return False
    ...
    def revise(self, assignment, i, j):
        ...
        # print i, j,  assignment[i]
        for val_i in assignment[i]:
            arcs = []
            for val_j in assignment[j]:
                arcs.append((val_i, val_j))

            if any([x in arcs for x in self.constraints[i][j]]):
                continue
...
path = "sudokus/"
for board in ["easy", "hard", "medium", "veryhard"]:
    print "BOARD {}".format(board)
    csp_sudoku = create_sudoku_csp(path + board + ".txt")
    solution = csp_sudoku.backtracking_search()
    print_sudoku_solution(solution)
    print "Failures: {} \nBacktracks: {}\n\n".format(csp_sudoku.num_failure, csp_sudoku.num_backtrack)


# csp_colors = create_map_coloring_csp()
# csp_colors.backtracking_search()
