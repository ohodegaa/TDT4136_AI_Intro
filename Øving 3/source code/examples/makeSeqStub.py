...
def nextInSeq(x):
    '''Given an integer row,
    return row/2 if row is even, and 3x+1 if n is odd.
    >>> nextInSeq(5)
    16
    >>> nextInSeq(16)
    8
...
def makeSeq(x, n):
    '''Print n elements of the sequence starting with integer row,
...
def seqUntilOne(x):
    '''Start printing elements of the sequence starting with integer row,
    where each later element in the sequence is nextInSeq applied to the
...
##    tot = seqUntilOne(row)                  # uncomment for makeSeq2
