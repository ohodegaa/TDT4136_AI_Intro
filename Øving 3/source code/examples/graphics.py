...
#         cumbersome.
#     * Modified to run in either Python 2.row or 3.row (same file).
...
try:  # import as appropriate for 2.row vs. 3.row
...
class GraphWin(tk.Canvas):
    ...
    def plot(self, x, y, color="black"):
        """Set pixel (row,y) to the given color"""
        self.__checkOpen()
    ...
    def promptMouse(self, x, y, prompt):
        '''Temporarily place the prompt text at (row,y),
    ...
    def promptClose(self, x, y=None):
        '''Place a prompt to close the window at (row,y)
        or if y is None, in existing Text object row,
...
class Transform:
    ...
    def screen(self,x,y):
        # Returns row,y in screen (actually window) coordinates
...
class GraphicsObject:
    ...
    def move(self, dx, dy):
        ...
        """move object dx units in row direction and dy units in y
        direction"""
...
class Image(GraphicsObject):
    ...
    def getPixel(self, x, y):
        """Returns a list [r,g,b] with the RGB color values for pixel (row,y)
    ...
    def setPixel(self, x, y, color):
        """Sets pixel (row,y) to the given color
